...
impl Config {
    ...
    pub fn get_bool(&self, name: &str) -> Result<bool,GitError> {
        let mut val: c_int = -1; 
        match unsafe { git2::git_config_get_bool(&mut val, self._get_ptr(), name.to_c_str().as_ptr()) } {
            0 => Ok(val == 1),
    ...
    pub fn get_string(&self, name: &str) -> Result<String,GitError> {
        ...
        unsafe {
            match git2::git_config_get_string(&mut p, self._get_ptr(), name.to_c_str().as_ptr()) {
    ...
    pub fn get_entry(&self, name: &str) -> Result<GitConfigEntry,GitError> {
        ...
        unsafe {
            if git2::git_config_get_entry(&mut entryptr, self._get_ptr(), name.to_c_str().as_ptr()) != 0 {
    ...
    pub fn iterator(&self) -> Result<GitConfigIterator,GitError> {
        ...
        if unsafe { git2::git_config_iterator_new(&mut iter, self._get_ptr()) } != 0 {
            return Err(get_last_error());
        }
...
impl Iterator<GitConfigEntry> for GitConfigIterator {
    fn next(&mut self) -> Option<GitConfigEntry> {
        let mut entryptr: *mut GitConfigEntryRaw = ptr::mut_null();
        let ret = unsafe { git2::git_config_next(&mut entryptr, self._iter) };
        if ret == 0 {
            return unsafe {
                let deref: GitConfigEntryRaw = *entryptr;
                Some(GitConfigEntry{
...
impl Drop for GitConfigPtr {
    fn drop(&mut self) {
        ...
        unsafe {git2::git_config_free(self._val)} 
    }
}
...
impl Drop for GitConfigIterator {
    fn drop(&mut self) {
        ...
        unsafe {git2::git_config_iterator_free(self._iter)} 
    }
}
